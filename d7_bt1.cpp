...
void is_complete(struct node *);
int node_count(struct node*);
...
void is_complete(struct node * root)
{
	if(root==NULL)return;
	else {
		int length = ( pow(2,max_depth(root)) - 1);
		int cnt = node_count(root);
		if(length==cnt){
			printf("given tree is complect binary tree...!\n");
		}
		else{
			printf("given tree is NOT COMPLECT binaryy tree \n ");
		}
...
int cnt = node_count(root);
void bt_to_ar(struct node * root, int ar[], int * i){
	if(root==NULL) return;
	
	ar[*i]=root->data;
	(*i)++;
	bt_to_ar(root->left,ar,i);
	bt_to_ar(root->right,ar,i);
}
...
{  
    ...
    int cnt = node_count(root);
    printf(" \n number of nodes or elements in bianry tree : %d \n",cnt);
    
    int* ar =(int*)malloc(cnt * sizeof(int));
    int i=0;
    printf("\ncalling..\n");
    bt_to_ar(root,ar,&i);
    ...
    int j;
	for(j=0; j<(node_count(root)) ;j++){
		printf(" %d",ar[j]);
	}
    
    bubble_sort(int ar[],node_count(root));
    j=0;
    for(j=0; j<(node_count(root)) ;j++){
		printf(" %d",ar[j]);
	}
